...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $id_dewan = $_GET['id_dewan']; // Get id_dewan from the URL
    ...
    if (isset($_FILES['imageUpload']) && $_FILES['imageUpload']['error'] == 0) {
        ...
        if (move_uploaded_file($file_tmp, $upload_dir . $new_file_name)) {
            ...
            $sql_select = "SELECT url_gambar FROM dewan_pic WHERE jenis_gambar = ? AND id_dewan = ?";
            ...
            $sql = "UPDATE dewan_pic SET url_gambar = ? WHERE jenis_gambar = ? AND id_dewan = ?";
